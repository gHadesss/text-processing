...
LABELS_BIO = (
    ["O"]
    + ["B_" + label for label in LABELS]
    + ["I_" + label for label in LABELS]
)


def ann2tuple(s):
    _, label, start, end, text = s.rstrip().split(maxsplit=4)
    return int(start), int(end), label, text
...
def load_seqs_from_dir(path):
    ...
    for file_path in Path(path).rglob("*.txt"):
        ...
        if len(tokens_map) > 2048:
            continue  # maximum sequence length for model is 2048
        labels_seq = []
        idx = 0
        for span in spans:
            if span[0] == span[1]:
...
class TransformersDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.label_seqs = [
            self.process_labels(labels, get_label2idx(LABELS_BIO)) for labels in label_seqs
        ]
...
model = AutoModelForTokenClassification.from_pretrained(
    "cointegrated/rubert-tiny2", num_labels=len(LABELS_BIO)
).cuda()
